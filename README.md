...
# View Jupyter Notebook ([here](https://github.com/104-wonohfor/Project-Predicting-Credit-Card-Approvals/blob/main/notebook.ipynb))
...
cc_apps_train.fillna(np.mean(cc_apps_train.iloc[[2,7,10,14]]), inplace=True) 
cc_apps_test.fillna(np.mean(cc_apps_test.iloc[[2,7,10,14]]), inplace=True)
...
cc_apps_test = cc_apps_test.reindex(columns=cc_apps_train.keys(), fill_value=0)
```
